...
te<-summary(q$c.tEval)
tn<-summary(q$e.tNoise)
tem<-summary(q$c.tEvalmasked)
tnm<-summary(q$e.tNoisemasked)
temp<-summary(q$e.tEvalmasked_p)
tnmp<-summary(q$e.tNoisemasked_p)
...
qcte<- data.frame('Name'=sum.Names,'Min.'=sum.min,'1st Qu.'=sum.fsq,'Median'=sum.med,'Mean'=sum.mean,'3rd Qu.'=sum.tsq,'Max.'=sum.max)
@
<<Noisesummary, echo=FALSE, results='asis'>>=
tabqNoise <- xtable(qcte, digits=c(3),caption='Summary of the evaluated time.', label='tab:Noise')
print(tabqNoise,type="latex", row.names=FALSE)
@
