...
The following calculations are done by calling data stored in a neo4j graph database from R. To insert data in this database, first use \rcode{main_analysis.py} to generate a serie of json file containing all the useful info on your new data. Then use the code \rcode{toDB.py} to import these data in the neo4j database -- before running the script, please read the head lines and make the necessary changes described there.
...
%graph=startGraph('http://localhost:7474/db/data', username='neo4j', %password='admin')#open a port to the db, 
...
\begin{itemize}
  ...
  \item {\it Te} end time
  \item {\it Tp_b} start time masked
  \item {\it Tp_e} end time masked
  \item {\it tNoise} Period in s. where noise was detected
  \item {\it tNoisemasked} Period in s. where noise was detected (masked between Tp_ b and Tp_e)
    \item {\it TEL}
    \item {\it Location}
\end{itemize}
\item {\bf TrainType}
...
\subsectino{Getting started in R}

We will import the measurements for all the algorithms. RNeo4j accepts \rcode{data.frames} as input, so we will give it that. In the following R-code, we will call all evaluations made with the algorithms in the database, all Passings attached to those evaluations and all trains types of those Passings.
