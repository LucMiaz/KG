...
class CaseCreatorWidget(DetectControlWidget):
    ...
    def extbrowsercall(self):
        ...
        try:
            ...
            for folder in os.walk(self.mesPath.as_posix()):
                for file in folder:
                    if 'info.html' in file:
                        self.infofolder=pathlib.Path(folder[0]).absolute()
                        pathtoinfo=self.infofolder.joinpath('info.html').absolute().as_uri()
                        try:
                            os.startfile(pathtoinfo)
                        except:
                            QtGui.QMessageBox.warning(self,'Info.html not found',"I could not find the info.html. Maybe you'll be luckier...")
    ...
    def load_cases(self, listofpaths):
        ...
        for pathtofile in listofpaths:#listofpaths contains Windows paths
            ...
            if extension=='json':
                ...
                self.mesPath=pathtofile
            if self.mesPath:
                self.case_to_analyse(ID,mic,self.mesPath, gauthor)
            else:
